...
from util.load_data import load_data_list, load_data_C, sample_by_step, sample_timestep_reshape
from config.Config import File202240Config, TrainConfig
from model.LSTM_attention2_model import get_LSTM_attention_model
...
for feature_name in feature_enum.all_feature_list:
    for time_steps in train_config.time_steps_feature_importance_list:
        for cv_index in range(10):
            ...
            (x_train, y_train, train_weight_dir), (x_valid, y_valid, valid_weight_dir) = load_data_C(train_data_path,
                                                                                                     train_data_path,
                                                                                                     feature_name)
            ...
            # load model
            model = get_LSTM_attention_model(
            ...
            # start train
            model_save_path = f'{p}/weights/202240_feature_importance/{model_name}_best≥C/time_steps={time_steps}'
            if not os.path.exists(model_save_path):
                os.makedirs(model_save_path)
            model_save_path = f'{model_save_path}/{model_name}_C_best_{cv_index}.h5'
